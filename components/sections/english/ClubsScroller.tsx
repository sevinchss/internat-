      ...
      <div className="container-x flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex size-12 items-center justify-center rounded-full border border-line text-ink transition-colors hover:border-ink disabled:cursor-default disabled:opacity-40 disabled:hover:border-line"
          >
          ...
          <li key={club.id} className="w-[80vw] max-w-[460px] shrink-0 snap-start sm:w-[420px] lg:w-[460px]">
            <article>
              <div className="relative">
                ...
                />
                <span aria-hidden="true" className="absolute left-4 top-4 rounded-full bg-paper/92 px-3 py-1 font-display text-sm tabular-nums text-ink backdrop-blur-sm">
                  {String(i + 1).padStart(2, "0")}
                </span>
              </div>
              <h3 className="mt-5 text-display-s text-ink">{pick(club.name, locale)}</h3>
              <p className="mt-2 max-w-[40ch] text-ink-2">{pick(club.text, locale)}</p>
              <p className="mt-3 flex items-center gap-2 text-sm font-semibold text-ink-3">
