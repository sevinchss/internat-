...
export function ExamTable({
  ...
  return (
    <section aria-labelledby="exam-title" className="container-x py-20 lg:py-28">
                  ...
                  {g.rows.map((r, i) => (
                    <tr key={r.section} className="align-top">
                      ...
                      <td className="px-4 py-4 sm:px-3 sm:py-5">
                        ...
                        <span aria-hidden="true" className="mt-3 block h-1.5 w-full max-w-[280px] overflow-hidden rounded-full bg-surface-2">
                          <span className="block h-full rounded-full" style={{ width: `${r.share * 100}%`, background: r.color }} />
                        </span>
                      </td>
                      <td className="px-4 py-4 text-right font-display text-xl tabular-nums text-ink sm:px-7 sm:py-5 sm:text-2xl">{r.questions ?? "—"}</td>
                    </tr>
                  ))}
                  <tr className="border-t border-dashed border-line bg-surface-2/50">
                    <th scope="row" className="px-4 py-4 text-sm font-semibold text-ink-2 sm:px-3">
                      {totalLabel}
                    </th>
                    <td className="px-4 py-4 text-right font-display text-xl tabular-nums text-primary-ink sm:px-7 sm:text-2xl">{g.total}</td>
                  </tr>
                </tbody>
              ))}
            </table>
