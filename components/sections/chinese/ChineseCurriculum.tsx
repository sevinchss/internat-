...
function HskArc({ g, label, sub }: { g: ChineseGrade; label: string; sub: string }) {
  ...
  return (
    <svg viewBox="0 0 400 400" role="img" aria-label={label} className="block h-auto w-full">
        ...
        return (
          <g key={s.level}>
            ...
            <text
              ...
              className="font-sans"
              style={{ fontSize: 15, fontWeight: state === "target" ? 700 : 500, fill: state === "todo" ? "var(--ink-3)" : "var(--ink)" }}
            >
              HSK {s.level}
            </text>
          </g>
      ...
      <text x={C} y={C - 14} textAnchor="middle" className="font-display" style={{ fontSize: 56, fill: "var(--ink)", letterSpacing: "-0.02em" }}>
        HSK {g.hsk}
      </text>
    ...
    <section aria-labelledby="zh-curriculum" className="bg-surface py-20 lg:py-28">
      <div className="container-x">
        ...
        <TabList
          ...
          value={value}
          onChange={setValue}
          className="mt-10 -mx-1 px-1"
          tabs={chineseGrades.map((x) => ({ id: String(x.grade), label: gradeLabel(x.grade, locale) }))}
        />

        <TabPanel idBase="zh-grades" id={value} className="mt-8 grid grid-cols-1 items-center gap-10 rounded-[28px] border border-line bg-paper p-6 sm:p-10 md:grid-cols-2 lg:gap-16">
          <div className="mx-auto w-full max-w-[400px]">
            <HskArc g={g} label={arcLabel} sub={gradeLabel(g.grade, locale)} />
          </div>
          <div>
            <p className="text-sm font-semibold text-ink-3">{gradeLabel(g.grade, locale)}</p>
            <p className="mt-2 font-display text-display-s text-ink">{pick(g.focus, locale)}</p>
            ...
            <dl className="mt-8 grid grid-cols-2 border-t border-line pt-5">
              <div>
                <dt className="text-sm text-ink-3">{pick(c.target, locale)}</dt>
                <dd className="mt-1 font-display text-xl text-accent-ink">HSK {g.hsk}</dd>
              </div>
        ...
        <p className="mt-5 text-sm text-ink-3">{pick(c.note, locale)}</p>
      </div>
    </section>
  );
}
