...
function CultureCard({ card, locale, large }: { card: Card; locale: string; large?: boolean }) {
  return (
    <article className={cn("group grid grid-cols-1 gap-5", large ? "" : "sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] sm:items-end lg:grid-cols-1")}>
      <div className="relative">
        <Photo
          slot={card.photo}
          sizes={large ? "(min-width: 1024px) 700px, 100vw" : "(min-width: 1024px) 460px, (min-width: 640px) 50vw, 100vw"}
        ...
        <span
          lang="zh-CN"
          aria-hidden="true"
          className="absolute right-3 top-3 rounded-[10px] bg-paper/92 px-2.5 py-1 font-hanzi text-lg text-ink backdrop-blur-sm [writing-mode:vertical-rl]"
        >
          {card.hanzi}
        </span>
      </div>
      <div>
        <h3 className={cn("text-ink", large ? "text-display-s" : "font-sans text-xl font-bold tracking-normal")}>{pick(card.title, locale)}</h3>
        <p className="mt-2 max-w-[48ch] text-ink-2">{pick(card.text, locale)}</p>
        <p className="mt-4 flex items-center gap-2 text-sm font-semibold text-ink-3">
...
export function CultureClubs() {
  ...
  return (
    <section aria-labelledby="zh-culture" className="border-t border-line py-20 lg:py-28">
      <div className="container-x">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <h2 id="zh-culture" className="text-display-m text-ink">
            {pick(c.title, locale)}
          </h2>
          <p className="max-w-[44ch] text-ink-2">{pick(c.lead, locale)}</p>
        </div>
