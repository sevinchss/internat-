...
let tx0 = W, tx1 = 0, ty0 = H, ty1 = 0;
for (let y = 0; y < H; y++) for (let x = Math.round(W * 0.55); x < W; x++) {
  if (isNavy(idx(x, y))) { ty0 = Math.min(ty0, y); ty1 = Math.max(ty1, y); tx1 = Math.max(tx1, x); }
}
// b) columns: within those rows, the leftmost navy pixel is where the wordmark starts
for (let y = ty0; y <= ty1; y++) for (let x = Math.round(W * 0.3); x < W; x++) {
  if (isNavy(idx(x, y))) { tx0 = Math.min(tx0, x); break; }
}
// pad the text box a little for anti-aliasing
const pad = 6;
const text = { x0: Math.max(0, tx0 - pad), y0: Math.max(0, ty0 - pad), x1: Math.min(W - 1, tx1 + pad), y1: Math.min(H - 1, ty1 + pad) };
...
for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) {
  ...
  if (data[idx(x, y) + 3] > 24) { ex0 = Math.min(ex0, x); ex1 = Math.max(ex1, x); ey0 = Math.min(ey0, y); ey1 = Math.max(ey1, y); }
}
...
await sharp(Buffer.from(data), rawOpts).trim({ threshold: 1 }).png({ compressionLevel: 9 }).toFile(path.join(OUT, "logo-full.png"));

// logo-full-dark.png — wordmark recoloured to white, emblem untouched
const dark = Buffer.from(data);
for (let y = text.y0; y <= text.y1; y++) for (let x = text.x0; x <= text.x1; x++) {
  const i = idx(x, y);
  if (dark[i + 3] > 0) { dark[i] = 255; dark[i + 1] = 255; dark[i + 2] = 255; }
}
await sharp(dark, rawOpts).trim({ threshold: 1 }).png({ compressionLevel: 9 }).toFile(path.join(OUT, "logo-full-dark.png"));

// logo-wordmark(-dark).png — just the "International Language School" text, for the loader
const tw = text.x1 - text.x0 + 1, th = text.y1 - text.y0 + 1;
await sharp(await sharp(Buffer.from(data), rawOpts).extract({ left: text.x0, top: text.y0, width: tw, height: th }).png().toBuffer()).trim({ threshold: 1 }).png().toFile(path.join(OUT, "logo-wordmark.png"));
await sharp(await sharp(dark, rawOpts).extract({ left: text.x0, top: text.y0, width: tw, height: th }).png().toBuffer()).trim({ threshold: 1 }).png().toFile(path.join(OUT, "logo-wordmark-dark.png"));
...
const mw = ex1 - ex0 + 1, mh = ey1 - ey0 + 1;
...
const mark = await sharp({ create: { width: side, height: side, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
...
async function markOn(size, bg, inset) {
  const inner = await sharp(mark).resize(Math.round(size * inset), Math.round(size * inset)).png().toBuffer();
  const off = Math.round((size - Math.round(size * inset)) / 2);
  return sharp({ create: { width: size, height: size, channels: 4, background: bg } }).composite([{ input: inner, left: off, top: off }]).png().toBuffer();
}
...
const entries = sizes.map((s, k) => {
  ...
  e.writeUInt16LE(1, 4); e.writeUInt16LE(32, 6); e.writeUInt32LE(pngs[k].length, 8); e.writeUInt32LE(offset, 12);
  offset += pngs[k].length; return e;
});
...
await sharp({ create: { width: 1200, height: 630, channels: 4, background: paper } })
  ...
  .png().toFile(path.join(OUT, "og-base.png"));
...
for (const [k, n] of buckets) {
  ...
  let h = 0;
  if (s > 0) {
    if (max === r) h = ((g - b) / s) % 6; else if (max === g) h = (b - r) / s + 2; else h = (r - g) / s + 4;
    h = (h * 60 + 360) % 360;
  }
  let fam = null;
